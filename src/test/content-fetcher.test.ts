...
describe('ContentFetcher', () => {
  ...
  it('should validate note structure with detailed results', () => {
    ...
    const noFrontMatterResult = ContentFetcher.validateRawNote(noFrontMatterNote);
  ...
  it('should validate ULID format using ulid library', () => {
    // 有效的 ULID
    expect(ContentFetcher.validateULID('01K5RR9NFREBCCRT4YHNN94W29')).toBe(true);
    expect(isValidULID('01K5RR9NFREBCCRT4YHNN94W29')).toBe(true);
    
    // 无效的 ULID - 太短
    expect(ContentFetcher.validateULID('01K5RR9NFREBCCRT4YHNN94W2')).toBe(false);
    expect(isValidULID('01K5RR9NFREBCCRT4YHNN94W2')).toBe(false);
    
    // 无效的 ULID - 包含无效字符 (I, L, O, U 不允许)
    expect(ContentFetcher.validateULID('01K5RR9NFREBCCRT4YHNN94W2I')).toBe(false);
